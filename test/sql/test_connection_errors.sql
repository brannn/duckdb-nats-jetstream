...
WITH telemetry_data AS (
    ...
    FROM nats_scan('telemetry', json_extract := ['device_id'])
),
...
-- SELECT * FROM nats_scan('telemetry', url := 'invalid://bad-url:9999') LIMIT 1;
...
-- SELECT * FROM nats_scan('telemetry', url := 'nats://localhost:9999') LIMIT 1;
